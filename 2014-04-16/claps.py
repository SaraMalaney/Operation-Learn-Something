...
def faceFind(im, imycrcb, detectedEyes, detectedFace, win):
    num_eyes = 0
    eyes_in_face = []
    actual_eyes = []
...
captureWin = "Capture"
cv2.namedWindow(captureWin, cv2.CV_WINDOW_AUTOSIZE)
...
imycrcb = cv2.cvtColor(flipped,cv.CV_RGB2YCrCb)
means, stds = faceFind(im, imycrcb, detectedEyes, detectedFace, captureWin)
...
imycrcb = cv2.inRange(imycrcb, low, high)
imycrcb  = blobSmoothing(imycrcb)
...
while True:
    ...
    imycrcb = cv2.inRange(imycrcb, low, high)
    imycrcb  = blobSmoothing(imycrcb)
    contours, hierarchy = cv2.findContours(imycrcb,cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE)
    ...
    cv2.drawContours(imycrcb, goodcontours, -1,(255,0,0),2)
